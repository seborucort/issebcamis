...
// ===== Producto destacado aleatorio en el HERO =====
document.addEventListener("DOMContentLoaded", () => {
  const destacado = productos[Math.floor(Math.random() * productos.length)];

  const heroImg = document.querySelector(".hero-card .preview-img");
  const heroName = document.querySelector(".hero-card strong");
  const heroPrice = document.querySelector(".hero-card .price");
  const heroSmall = document.querySelector(".hero-card .small");

  if (heroImg && heroName && heroPrice && heroSmall) {
    heroImg.src = destacado.img;
    heroImg.alt = destacado.name;
    heroName.textContent = destacado.name;
    heroPrice.textContent = `$${destacado.price} MXN`;
    heroSmall.textContent = "Camiseta del momento";
    console.log("✅ Producto destacado:", destacado.name);
  } else {
    console.log("⚠️ No se encontraron los elementos del hero.");
  }
});
